...
export const SearchAndFilters: React.FC<SearchAndFiltersProps> = ({
  ...
  return (
    <div id="search-filter-section" className="bg-white border border-stone-200 rounded-2xl p-3.5 sm:p-6 shadow-sm mb-6 sm:mb-8 space-y-3.5 sm:space-y-4">
      {/* Search Input Bar */}
      <div className="relative">
        ...
        <input
          ...
          placeholder="Cerca ortaggio, frutto o aroma (es. Pomodoro, Basilico, Peronospora...)"
          className="w-full pl-9 sm:pl-11 pr-9 sm:pr-10 py-2.5 sm:py-3.5 rounded-xl border border-stone-300 bg-stone-50/70 text-stone-900 placeholder-stone-400 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent focus:bg-white text-sm sm:text-base transition"
        />
        {filters.ricerca && (
          <button
            id="btn-clear-search"
            onClick={handleClearSearch}
            className="absolute inset-y-0 right-0 pr-3 sm:pr-3.5 flex items-center text-stone-400 hover:text-stone-600"
      ...
      {/* Quick Search Chips */}
      <div className="flex items-center gap-1.5 text-xs overflow-x-auto no-scrollbar py-0.5 -mx-3.5 px-3.5 sm:mx-0 sm:px-0 sm:flex-wrap">
        <span className="text-stone-400 font-medium text-[11px] shrink-0">Frequenti:</span>
        {[
          { label: '🍅 Pomodori', query: 'pomodori' },
          { label: '🌿 Aromatiche', query: 'erbe aromatiche' },
          { label: '🍎 Alberi frutto', query: 'alberi da frutto' },
          { label: '🌱 Legumi', query: 'legumi' },
          { label: '🍓 Frutti & Bacche', query: 'frutta' },
          { label: '🥔 Patate', query: 'patata' },
          ...
          <button
            ...
            className={`px-2.5 py-1 rounded-lg transition text-xs font-medium shrink-0 ${
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2.5 sm:gap-3 pt-1">
        {/* Category Pills - Smooth horizontal scroll on mobile */}
        <div id="category-pills" className="flex items-center gap-1.5 sm:gap-2 overflow-x-auto no-scrollbar -mx-3.5 px-3.5 sm:mx-0 sm:px-0 sm:flex-wrap">
          <button
            id="cat-pill-tutti"
            onClick={() => handleCategoryChange('tutti')}
            className={`px-3 py-1.5 rounded-full text-xs sm:text-sm font-medium transition shrink-0 ${
              filters.categoria === 'tutti'
          ...
          <button
            ...
            className={`px-3 py-1.5 rounded-full text-xs sm:text-sm font-medium transition flex items-center space-x-1 shrink-0 ${
          ...
          >
            <span>🌿</span>
            <span>Aromatiche</span>
          </button>
        </div>

        {/* Sort and Filters helper */}
        <div className="flex items-center justify-between sm:justify-end space-x-2 text-xs sm:text-sm pt-1 sm:pt-0 border-t border-stone-100 sm:border-t-0">
          ...
          <select
            ...
            className="bg-stone-100 border border-stone-300 rounded-lg px-2 py-1 sm:px-2.5 sm:py-1.5 text-stone-700 font-medium focus:outline-none focus:ring-1 focus:ring-emerald-500 text-xs sm:text-sm"
          >
            <option value="nome">Nome (A - Z)</option>
            <option value="giorniRaccolto">Raccolto più rapido</option>
      ...
      <div className="pt-2 border-t border-stone-100 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2.5 sm:gap-3 text-xs sm:text-sm">
        {/* Month Selector for Sowing */}
        <div className="flex flex-wrap items-center gap-1.5 sm:gap-2">
          <span className="text-stone-500 font-medium flex items-center gap-1 text-[11px] sm:text-xs">
          ...
          >
            <span>🎯</span>
            <span>{MONTH_NAMES[currentRealMonth - 1]} (Ora)</span>
          </button>
          ...
          >
            ...
            {MONTH_NAMES.map((m, idx) => (
              <option key={idx} value={idx + 1}>
                {m} {idx + 1 === currentRealMonth ? '(Ora)' : ''}
              </option>
            ))}
          </select>

          {/* Difficulty filter */}
          <div className="flex items-center space-x-1 pl-1 sm:pl-2 border-l border-stone-200">
        ...
        <div className="flex items-center justify-between sm:justify-end space-x-3 text-xs text-stone-500 pt-1 sm:pt-0">
          <span>
            Mostrati <strong>{totalResults}</strong> su {totalPlants}
          </span>
          {hasActiveFilters && (
            <button
