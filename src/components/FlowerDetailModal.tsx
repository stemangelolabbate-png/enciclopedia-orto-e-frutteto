...
export const FlowerDetailModal: React.FC<FlowerDetailModalProps> = ({ flower, onClose }) => {
  ...
  return (
    <div 
      ...
      className="fixed inset-0 z-50 overflow-y-auto bg-stone-950/70 backdrop-blur-xs flex items-center justify-center p-3 sm:p-5 animate-in fade-in duration-200"
      onClick={onClose}
    >
      <div 
        id="flower-detail-modal-container"
        className="relative w-full max-w-3xl bg-stone-900 border border-stone-800 rounded-2xl shadow-2xl overflow-hidden text-stone-100 my-auto"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Modal Header Banner */}
        <div className="relative bg-gradient-to-r from-pink-950/80 via-purple-950/80 to-stone-900 p-5 sm:p-6 border-b border-stone-800">
          <button
            id="btn-close-flower-modal"
            onClick={onClose}
            className="absolute top-4 right-4 p-2 rounded-xl bg-stone-800/80 hover:bg-stone-700 text-stone-300 hover:text-white transition"
            title="Chiudi scheda"
          >
            <X className="w-5 h-5" />
          </button>
          ...
          <div className="flex flex-wrap items-start gap-4">
            <div className="w-16 h-16 sm:w-20 sm:h-20 rounded-2xl bg-stone-900/90 border border-pink-500/30 flex items-center justify-center text-4xl sm:text-5xl shadow-lg shrink-0">
              {flower.icona}
            </div>
            <div className="flex-1 min-w-[240px]">
              <div className="flex flex-wrap items-center gap-2 mb-1">
                <span className="px-2.5 py-0.5 rounded-full text-xs font-semibold bg-pink-900/60 text-pink-300 border border-pink-700/50">
                  {flower.tipo}
                </span>
              ...
              <p className="text-sm italic text-pink-200/80 font-sans mt-0.5">
              ...
              <div className="flex flex-wrap items-center gap-1.5 mt-3">
                {flower.attiraImpollinatori && (
                  <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-md text-xs font-medium bg-amber-950/70 text-amber-300 border border-amber-800/60">
                    🐝 Attira Impollinatori
                  </span>
                )}
                {flower.edule && (
                  <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-md text-xs font-medium bg-emerald-950/70 text-emerald-300 border border-emerald-800/60">
                    <Utensils className="w-3 h-3" /> Fiore Commestibile
                  </span>
                )}
                {flower.amicoOrto && (
                  <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-md text-xs font-medium bg-cyan-950/70 text-cyan-300 border border-cyan-800/60">
                    <ShieldCheck className="w-3 h-3" /> Amico dell'Orto
                  </span>
                )}
                {flower.profumata && (
                  <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-md text-xs font-medium bg-rose-950/70 text-rose-300 border border-rose-800/60">
                    <Sparkles className="w-3 h-3" /> Fiore Profumato
                  </span>
                )}
              </div>
            </div>
